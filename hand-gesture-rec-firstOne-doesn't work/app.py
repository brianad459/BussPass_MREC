...
def logging_csv(number, mode, landmark_list, point_history_list):
    if mode == 1 and (0 <= number <= 9):
        ...
        print(f'👉 Logging keypoints: label={number} to {csv_path}')
        try:
            with open(csv_path, 'a', newline="") as f:
                writer = csv.writer(f)
                writer.writerow([number, *landmark_list])
        except Exception as e:
            print(f'⚠️ Error writing keypoint CSV: {e}')
    elif mode == 2 and (0 <= number <= 9):
        csv_path = 'model/point_history_classifier/point_history.csv'
        print(f'👉 Logging point history: label={number} to {csv_path}')
        try:
            with open(csv_path, 'a', newline="") as f:
                writer = csv.writer(f)
                writer.writerow([number, *point_history_list])
        except Exception as e:
            print(f'⚠️ Error writing point history CSV: {e}')
